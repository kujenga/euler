...
public class MinPathSum {
	...
	public static void main(String[] args) {
		...
		System.out.println("minPath: " + minPathRD(matrixIn, M_SIZE));
	}
	...
	private static int minPathRD(int matrix[][], int size) {
		...
		while (!((next.getX() == (size - 1)) && (next.getY() == (size - 1)))) {
			...
			next = new Node((int) nextP.getX(), (int) nextP.getY(),
					cur.getPath() + matrix[(int) nextP.getX()][(int) nextP.getY()]);
...
class Node implements Comparable {
	...
	public String toString() {
		return "x:" + xValue + " y:" + yValue + " pathValue:" + pathValue;
	}
}
